...
function nodes_controller()
{
    ...
    $result = false;
    if (!$session['read']) return array('content'=>$result);
    ...
    } elseif ($route->action == 'apidocs' && $session['write']) {
        $route->format = "html";
    ...
    if ($route->method == 'GET' || $route->method == 'POST' || $route->method == 'DELETE')
    {
        ...
        if ($route->method == 'POST' && $session['write'])
        {
                ...
                if ($prop=="values") {
                    ...
                    if ($rxtx!==false) {
                        ...
                        if ($rxtx=="tx") {
                            ...
                            if(!$mqtt->connect(true,NULL,$mqtt_server['user'],$mqtt_server['password'])) exit(1);
        ...
        if ($route->method == 'GET' && $session['read'])
        {
            $nodes = json_decode($redis->get("nodes"));
            ...
            if ($nodeid===false && $varid===false && $prop===false)
            {
                foreach ($nodes as $nid=>$node) {
                    if (isset($nodes->$nid->rssi)) $config->$nid->rssi = $nodes->$nid->rssi;
